...
TEST_ATR = 1.5
TEST_ADX = 35
SLIPPAGE = 0.0002        # 正常滑点: 0.02%
FUNDING_RATE = 0.0001    # 资金费率: 0.01%
TRADING_FEE = 0.0005     # 手续费: 0.05%
...
class Backtester:
    ...
    def run_strategy(self, df, atr_stop, adx_limit):
        ...
        for i in range(50, len(df)):
            ...
            if self.position != 0:
                self.manage_position(curr, price, atr_stop)
            elif self.position == 0:
                self.check_entry(last, price, atr_stop, adx_limit)

    def check_entry(self, last, price, atr_stop, adx_limit):
        ...
        if (adx > adx_limit) or (adx_slope > 1.5):
            if vol > (vol_ma * 1.2):
                if is_bull and rsi > 50: self.open_position(1, price, last['ATR'], atr_stop)
                elif is_bear and rsi < 50: self.open_position(-1, price, last['ATR'], atr_stop)

    def open_position(self, direction, price, atr, atr_stop):
    ...
    def manage_position(self, row, price, atr_stop):
        atr = row['ATR']
        ...
        if is_sl_hit:
            ...
            if random.random() < FAULT_PROB:
                if not self.is_hardened:
                    ...
                    self.close_position(penalty_price, is_partial=False)
                    self.fatal_errors += 1
                    return
                else:
                    # 加固模型：Watchdog 介入，按原价成功止损
                    self.watchdog_interventions += 1
                    self.close_position(self.sl_price, is_partial=False)
                    return
            else:
                self.close_position(self.sl_price, is_partial=False)
                return
        ...
        if not self.partial_tp_done:
            ...
            if profit > (atr * 2.0):
                self.close_position(price, is_partial=True)
                self.sl_price = self.entry_price
                self.is_break_even = True
                return 

    def close_position(self, price, is_partial=False):
